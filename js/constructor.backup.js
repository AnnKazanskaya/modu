...
//  Тёплая «уютная» подача (в духе Paralives): текстуры ткани,
//  мягкое отражённое освещение, контактная тень, плавные формы.
// ============================================================
...
// roughness / sheen / сила нормалей / повтор текстуры
const MATERIALS = {
  boucle: { normal: 0.7,  rep: 7,  tex: 'boucle' },
  linen:  { normal: 0.5,  rep: 11, tex: 'linen'  },
  velvet: { normal: 0.22, rep: 6,  tex: 'boucle' },
};
...
// мягкие тёплые контуры — «нарисованный» вид как в игре
const effect = new OutlineEffect(renderer, {
  defaultThickness: 0.0032,
  defaultColor: [0.20, 0.13, 0.07],
  defaultAlpha: 0.55,
  defaultKeepAlive: true,
});

// ступенчатая «мультяшная» закраска (cel-shading)
function makeToonRamp() {
  const steps = [120, 178, 214, 244];
  const c = document.createElement('canvas'); c.width = steps.length; c.height = 1;
  const x = c.getContext('2d');
  steps.forEach((v, i) => { x.fillStyle = `rgb(${v},${v},${v})`; x.fillRect(i, 0, 1, 1); });
  const t = new THREE.CanvasTexture(c);
  t.minFilter = t.magFilter = THREE.LinearFilter;   // мягкие переходы, не резкие
...
// ---------- Тёплый свет ----------
scene.add(new THREE.AmbientLight(0xfff0db, 0.55));     // тёплый общий свет — уютно, без тёмных провалов
const hemi = new THREE.HemisphereLight(0xfff4e6, 0xc4b094, 0.35);
scene.add(hemi);

const key = new THREE.DirectionalLight(0xffe7c6, 1.7);  // ключ задаёт направление cel-закраски
...
const fill = new THREE.DirectionalLight(0xe9eeff, 0.25);
fill.position.set(-5, 3, -1);
scene.add(fill);

// ---------- Пол ----------
const floorMat = new THREE.MeshToonMaterial({ color: 0xece0cc, gradientMap: toonRamp });
floorMat.userData.outlineParameters = { visible: false };   // у пола контур не нужен
...
function fabricMaterial() {
  ...
  currentFabric = new THREE.MeshToonMaterial({
    color: new THREE.Color(COLORS[state.colorIndex].hex),
    gradientMap: toonRamp,
    normalMap: nrm,
    normalScale: new THREE.Vector2(m.normal, m.normal),   // лёгкая фактура ткани, без реализма
  });
  return currentFabric;
}
const legMaterial = new THREE.MeshToonMaterial({ color: 0x5a4230, gradientMap: toonRamp });
